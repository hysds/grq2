...
def migrate(index_name):
    ...
    r = requests.post(
        'http://localhost:9200/%s/_search?search_type=scan&scroll=60m&size=100' % src, data=json.dumps(query))
    scan_result = r.json()
    ...
    while True:
        r = requests.post(
            'http://localhost:9200/_search/scroll?scroll=60m', data=scroll_id)
        res = r.json()
