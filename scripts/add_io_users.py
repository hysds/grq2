...
def get_add_users():
    ...
    while not USER_RE.match(user):
        ...
        user = eval(input("Enter in space-separated user(s) to add:"))
    user = user.split()
    return user
...
def add_to_ios(users, hysdsio, existing):
    ...
    if set(users).union(set(existing[hysdsio])) == set(existing[hysdsio]):
        print("Supplied users already have permission to run {0}, nothing to do, Skipping.".format(
            hysdsio))
        return
    ...
    sure = ""
    while sure == "":
        sure = eval(input("Are you sure you want to add {0} to allowed users for '{1}' for a final user set of {2}?" .format(
