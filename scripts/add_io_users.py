...
def add_to_ios(users, hysdsio, existing):
    ...
    if set(users).union(set(existing[hysdsio])) == set(existing[hysdsio]):
        print("Supplied users already have permission to run {}, nothing to do, Skipping.".format(
            hysdsio))
        return
    ...
    sure = ""
    while sure == "":
        sure = eval(input("Are you sure you want to add {} to allowed users for '{}' for a final user set of {}?" .format(
