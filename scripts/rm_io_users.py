...
def get_rm_users():
    ...
    while not USER_RE.match(user):
        ...
        user = raw_input("Enter in space-separated user(s) to remove:")
    user = user.split()
    return user
def rm_to_ios(users, hysdsio, existing):
    ...
    if set(existing[hysdsio]) - set(users) == set(existing[hysdsio]):
        print("Supplied users don't have permission to run {0}, nothing to do, Skipping.".format(hysdsio))
        return
    ...
    sure = ""
    while sure == "":
        sure = raw_input("Are you sure you want to remove {0} to allowed users for '{1}' for a final user set of {2}?" .format(users,hysdsio,json.dumps(existing[hysdsio])))
