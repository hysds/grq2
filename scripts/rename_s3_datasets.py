...
# query
r = requests.get(
    'http://localhost:9200/grq_aria/_search?search_type=scan&scroll=10m&size=100')
...
while (True):
    r = requests.post(
        'http://localhost:9200/_search/scroll?scroll=10m', data=scroll_id)
    datasets = r.json()
            ...
            for url in updated_doc['_source'][url_key]:
                if 's3-browse.jpl.nasa.gov' in url:
                    ...
                    if (parsed_url.scheme == 's3') or ('hysds-aria-products.s3' in parsed_url.hostname):
                        date = updated_doc['_source']['starttime'].split('T')[
                            0].split('-')
                        ...
                        if parsed_url.scheme == 's3':
                            ...
                            new_urls.append(
                                parsed_url.scheme + "://" + parsed_url.hostname + '/' + path)
